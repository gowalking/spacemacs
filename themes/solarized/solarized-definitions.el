...
(defvar solarized-colors           ; ANSI(Solarized terminal)
  ...
  '((base03  "#002b36" "#042028" "#1c1c1c" "brightblack"   "black")
    ...
    (lblue   "#268BD2" "#268BD2" "#268BD2" "blue"          "blue")
    (dblue   "#3963D0" "#3963D0" "#3963D0" "dblue"         "dblue")
  ...
  (let ((bold        (if solarized-bold 'bold        'unspecified))
    ...
    (let ((bg-back   '(:background back))
      ...
      (mapcar (lambda (face) (apply 'create-face-spec face))
              `(;; basic
                (default (,nil ,nil))   ; Normal
                (cursor (,@fg-base03 ,@bg-base2))  ; Cursor
                (shadow (,@fg-base01))
                (link (,@fmt-undr ,@fg-violet))    ; Underlined
                (link-visited (,@fmt-undr ,@fg-magenta))
                (match (,@fmt-revr ,nil))   ; Search
                ...
                ;; show-paren - MatchParen 括号显示样式
                (show-paren-match (,@fmt-bold ,nil ,nil))
                (show-paren-mismatch (,@fmt-bold ,nil ,nil))
                ...
                ;; company 自动补全弹出框配色
                (company-tooltip (,@fg-base3 ,@bg-lblue))
                (company-tooltip-selection (,@fg-base3 ,@bg-yellow))	;;所选行
                (company-tooltip-mouse (,@fg-base3 ,@bg-yellow))
                (company-tooltip-common (,@fg-base3 ,@bg-blue ,nil))
                (company-tooltip-common-selection (,@fg-base3 ,@bg-yellow ,@fmt-bold)) ;;每行的类名称
                (company-tooltip-annotation (,@fg-base3 ,nil))
                (company-scrollbar-fg (,@bg-yellow))	;;滚动条前景色
                (company-scrollbar-bg (,nil))	;;滚动条背景色
                (company-preview (,@bg-green))
                (company-preview-common (,@bg-yellow))
